...
#>
  ...
  begin {
    ...
    Out-Verbose "Source = [$($Source.IPAddressToString)] SubnetMask = [$($SubnetMask.IPAddressToString)] SourceNetwork = [$(([ipaddress] $SourceResult).IPAddressToString)]"
  }

  process {
    foreach ($curTarget in $Target) {
      $TargetResult = $curTarget.Address -band $SubnetMask.Address
      Out-Verbose "Target = [$($curTarget.IPAddressToString)] SubnetMask = [$($SubnetMask.IPAddressToString)] TargetNetwork = [$(([ipaddress] $TargetResult).IPAddressToString)]"
