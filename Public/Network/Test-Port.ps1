      ...
      foreach ($p in $port) {
        if ($tcp) {
          ...
          if (!$wait) {
            #Close connection
            $tcpobject.Close()
            Out-Verbose 'Connection Timeout'
            #Build report
            $temp.ComputerName = $c
            $temp.Port = $p
            $temp.Protocol = 'TCP'
        ...
        if ($udp) {
          #$temp = '' | Select-Object -Property ComputerName, Protocol, Port, Open, Notes
          $temp = New-Object -TypeName psobject -Property @{ Computername = ''; Protocol = ''; Port = 0; Open = $false; Notes = '' }
          Out-Verbose 'Making UDP connection to remote server'
          ...
          try {
            ...
            $Buffer[0] = 27
            Out-Verbose 'Sending message to remote host'
            ...
            Out-Verbose 'Communication failed'
