...
#>
  ...
  begin {
    ...
    function Get-LocalIpNetwork {
      Get-WmiObject -Class Win32_NetworkAdapterConfiguration -Filter "IPEnabled = 'True'" | Where-Object { $_.IPAddress -and $_.IPSubnet } | ForEach-Object {
    ...
    function New-KnownGoodWolProxy {
    ...
    function Remove-KnownGoodWolProxy ($IpAddress) {
      $NewCsvContents = Import-Csv -Path $KnownGoodWolProxyHostsFilePath | Where-Object { $_.IpAddress -ne $IpAddress }
    ...
    function Get-KnownGoodWolProxy ([string]$IpNetwork) {
      if (!(Test-Path $KnownGoodWolProxyHostsFilePath)) {
        ...
        $Hosts = Import-Csv -Path $KnownGoodWolProxyHostsFilePath | Where-Object { $_.IpNetwork -eq $IpNetwork }
        if (!$Hosts) {
          Write-Verbose "No known good WOL proxy hosts found in IP network '$IpNetwork'"
          $false
        } else {
          $HostsOnNet = $Hosts | Where-Object { $_.IpNetwork -eq $IpNetwork }
          if ($HostsOnNet) {
            Write-Verbose "$(($HostsOnNet | Measure-Object -Sum -ea silentlycontinue).Count) (unknown accessibility) known good WOL proxy hosts found on IP network '$IpNetwork'"
            Write-Verbose 'Checking previously known good WOL proxy hosts if still usable'
            $AccessibleHost = $HostsOnNet | Where-Object { Validate-IsValidHost $_.IpAddress } | Select-Object -First 1 -ExpandProperty IpAddress
            if ($AccessibleHost) {
