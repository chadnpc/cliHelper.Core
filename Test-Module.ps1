...
.EXAMPLE
    ...
    Will test the module in .\BuildOutput\cliHelper.core\0.2.9\
.EXAMPLE
    .\Test-Module.ps1
    Will test the latest  module version in .\BuildOutput\cliHelper.core\0.2.9
#>
param (
...
begin {
  ...
  $manifestFile = [IO.FileInfo]::New([IO.Path]::Combine($BuildOutDir.FullName, "cliHelper.core.psd1"))
  ...
  if (!(Test-Path -Path $([IO.Path]::Combine($PSScriptRoot, "cliHelper.core.psd1")) -PathType Leaf -ErrorAction Ignore)) { throw [System.IO.FileNotFoundException]::New("Module manifest file Was not Found in '$($BuildOutDir.FullName)'.") }
